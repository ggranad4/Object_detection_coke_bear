...
def converter(filename, input):
    if(input == 1):
        ...
        for f1 in files:
            if(os.path.isdir(f1)):
                converter(os.path.dirname(f1) + '/' + os.path.basename(f1),1)
            try:
        ...
        new_path = os.path.dirname(filename) + '/' +  os.path.basename(filename) + '_Converted_to_Jpgs'
        print(new_path)
        if not os.path.exists(new_path):
            os.makedirs(new_path)
        for f1 in files:
            if(os.path.isdir(f1)):
                converter(os.path.dirname(f1) + '/' + os.path.basename(f1), 0)
            try:
